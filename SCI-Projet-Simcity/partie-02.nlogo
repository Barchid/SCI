...
to setup
  ...
  ; Création des houses
  create-houses nbHouses [init-house]
...
; Fonction appelée quand une voiture arrive devant une maison
to car-interact-house [near-local]
  if near-local = local [
    ask near-local [set occupation (occupation + 1)]
    die
  ]
end
...
to init-house
  ...
  set occupation (random (nbOccupation) + 1)
...
to house-decide
  ...
  if occupation > 0 and (random carFrequence) = 0 [
