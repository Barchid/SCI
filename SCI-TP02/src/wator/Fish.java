...
public class Fish extends Agent {
	...
	public void decide() {
		...
		// IF [fish can fuck and move in the same turn]
		boolean canDoTwoActions = this.appConfig.getWatorBehavior() == WatorAppConfig.FUCK_AND_MOVE
				|| this.appConfig.getWatorBehavior() == WatorAppConfig.FUCK_EAT_AND_MOVE;
		if (canDoTwoActions) {
		...
		// ELSE (fish can fuck or move but not both at the same time)
		else {
			// Fuck in priority at the new coordinates
			if(canFuck) {
				this.fuck(coordinates[0], coordinates[1]);
			}
			// Or move and decrement breedtime
			else {
				this.environment.moveAgent(this, coordinates[0], coordinates[1]);
				this.breedTime = this.breedTime > 0 ? this.breedTime - 1 : this.breedTime;
			}
